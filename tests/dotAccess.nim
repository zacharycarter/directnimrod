...
var c: array[4,float32]
d3d11.`.`(ctx, ClearRenderTargetView, rtv, c)
proc Update()
...
proc main() =
  ...
  result = swapChain.GetBuffer(0, addr IID_ID3D11Texture2D, cast[ptr LPVOID](addr pBackBuffer))
  if result != S_OK:
  ...
  result = device.CreateRenderTargetView(cast[ptr ID3D11Resource](pBackBuffer), addr rtvdesc, addr rtv)
  if result != S_OK:
    quit("could not create the render target view " & $result)
  ctx.OMSetRenderTargets(1, addr rtv, nil)
